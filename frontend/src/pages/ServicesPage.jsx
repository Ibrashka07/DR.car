    ...
    <div style={{ paddingTop: '64px', minHeight: '100vh' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '3rem 2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '2rem', alignItems: 'center' }}>
          ...
          <button onClick={() => setSelectedCat(null)} style={filterBtn(!selectedCat)}>Все</button>
          {categories.map(c => (
            <button key={c.id} onClick={() => setSelectedCat(c.title)} style={filterBtn(selectedCat === c.title)}>
              {c.title}
            </button>
          ))}
        </div>
            ...
            {services.map(s => (
              ...
              >
                <div style={{ height: 200, background: '#1a1a1a', overflow: 'hidden' }}>
                  {s.photo
                    ? <img src={`http://127.0.0.1:8000${s.photo}`} alt={s.title}
                        style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem' }}>🔧</div>
                  }
                </div>
                <div style={{ padding: '1.25rem' }}>
                  <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.3)', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '0.4rem' }}>
                    {s.category_title}
                  </p>
                  <h3 style={{ fontWeight: 700, fontSize: '1rem', marginBottom: '0.75rem', lineHeight: 1.3 }}>{s.title}</h3>
