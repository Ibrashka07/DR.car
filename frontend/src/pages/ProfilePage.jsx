...
export default function ProfilePage() {
  ...
  useEffect(() => {
    api.get('/my-cars/').then(r => setCars(r.data.results ?? r.data)).catch(() => {});
  }, []);
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '3rem 2rem', display: 'flex', gap: '2.5rem', alignItems: 'flex-start' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.8rem' }}>@{user?.username}</p>
          </div>
          ...
          {[
            ...
            <button key={id} onClick={() => setTab(id)} style={{
              ...
              borderRadius: '8px', color: tab === id ? 'white' : 'rgba(255,255,255,0.5)',
        ...
        <div style={{ flex: 1 }}>
          ...
          {tab === 'cars' && (
            <div>
              <h2 style={{ fontWeight: 700, fontSize: '1.5rem', marginBottom: '1.5rem' }}>Мои автомобили</h2>
              {cars.length === 0 ? (
                <div style={{
                  background: '#111', border: '1px solid rgba(255,255,255,0.07)',
                  ...
                  {cars.map(car => {
                    ...
                    return (
                      <div key={car.id} style={{
                        ...
                        {car.photo
                          ? <img src={`http://127.0.0.1:8000${car.photo}`} alt=""
                              style={{ width: 100, height: 70, objectFit: 'cover', borderRadius: '8px', flexShrink: 0 }} />
                          : <div style={{ width: 100, height: 70, background: '#1a1a1a', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem', flexShrink: 0 }}>🚗</div>
                        }
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.35rem 0.9rem', borderRadius: '20px', background: `${cfg.color}20`, border: `1px solid ${cfg.color}40` }}>
                            <span>{cfg.icon}</span>
                            <span style={{ color: cfg.color, fontSize: '0.85rem', fontWeight: 600 }}>{cfg.label}</span>
                          </div>
                          {car.status_note && (
          ...
          {tab === 'cart' && (
            <div>
              <h2 style={{ fontWeight: 700, fontSize: '1.5rem', marginBottom: '1.5rem' }}>Корзина</h2>
              {cartItems.length === 0 ? (
                <div style={{
                  background: '#111', border: '1px solid rgba(255,255,255,0.07)',
                  borderRadius: '12px', padding: '3rem', textAlign: 'center', color: 'rgba(255,255,255,0.3)',
                }}>
                  <p style={{ fontSize: '2rem', marginBottom: '1rem' }}>🛒</p>
                  <p>Корзина пуста</p>
                </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem' }}>
                    {cartItems.map(item => (
                      <div key={item.id} style={{
                        ...
                        {item.product.photo
                          ? <img src={`http://127.0.0.1:8000${item.product.photo}`} alt=""
                              style={{ width: 70, height: 70, objectFit: 'cover', borderRadius: '8px', flexShrink: 0 }} />
                          : <div style={{ width: 70, height: 70, background: '#1a1a1a', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', flexShrink: 0 }}>🔧</div>
                        }
                        <div style={{ flex: 1 }}>
                          {item.product.brand && (
                            <p style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)', letterSpacing: '1px', textTransform: 'uppercase' }}>{item.product.brand}</p>
                          )}
                        ...
                        <div style={{ textAlign: 'right', minWidth: 90 }}>
                          <p style={{ fontWeight: 700 }}>{Number(item.total_price).toLocaleString('ru-RU')} ₽</p>
          ...
          {tab === 'settings' && (
            <div>
              <h2 style={{ fontWeight: 700, fontSize: '1.5rem', marginBottom: '1.5rem' }}>Настройки профиля</h2>
              {saved && <p style={{ color: '#10b981', marginBottom: '1rem' }}>✓ Сохранено</p>}
              <form onSubmit={handleSave} style={{
                background: '#111', border: '1px solid rgba(255,255,255,0.07)',
                borderRadius: '16px', padding: '2rem',
                display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem',
              }}>
                {[['Имя', 'first_name'], ['Фамилия', 'last_name'], ['Email', 'email'], ['Телефон', 'phone']].map(([label, field]) => (
                  <div key={field}>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={labelInputStyle}>О себе</label>
...
const qtyBtn = {
  width: 28, height: 28, border: '1px solid rgba(255,255,255,0.12)',
